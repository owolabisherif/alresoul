...
import QuranScriptController from '@/actions/App/Http/Controllers/Api/QuranScriptController';
...
import QuranRecitersController from '@/actions/App/Http/Controllers/QuranRecitersController';
import { List, PauseIcon, Play, PlayIcon } from 'lucide-react';
...
import { index as quranChapers } from '@/routes/quran';
...
type ScriptType = {
    [key: number]: QurancScript;
};

type PlayerEvent = 'ended' | 'timeupdate' | 'play' | 'pause' | 'playing';

export default function Quran({ slug }: Prop) {
    ...
    const [words, setWords] = useState<ScriptVerse[]>([]);
    ...
    useEffect(() => {
        if (!data) {
            ...
            let selected = data['chapters'].find(
                (item) => useSlug(item.name_simple.toLowerCase()) == slug,
            );
    ...
    const handleFetchData = async () => {
        try {
            ...
            const dataRes = (await response.json()) as QuranResource;

            let selected = dataRes['chapters'].find(
                (item) => useSlug(item.name_simple.toLowerCase()) == slug,
            );
    ...
    useEffect(() => {
        ...
        const selected =
            useSlug((chapter as Chapter)?.name_simple?.toLowerCase()) == slug;

        if (selected) {
            getReciters();

            let start = (chapter as Chapter).pages[0];
            let end = (chapter as Chapter).pages[1];
            let length = end - start + 1;

            let pageItems = Array.from({ length: length }, (v, i) =>
                (start + i).toString(),
            );
    ...
    useEffect(() => {
        ...
        if (parseInt(lastpage!) != end) return;

        let p = pages[currentIndex];
        setCurrentPage(parseInt(p));
        handleGetScript(p);
        setRenderedPages([parseInt(p)]);
    }, [pages]);
    ...
    useEffect(() => {
        ...
        setRenderedPages((val) => [
            ...new Set([...val, parseInt(pages[currentIndex])]),
        ]);

        handleGetScript(pages[currentIndex]);
    }, [visible, currentIndex]);

    useEffect(() => {
        if (!player.current || !audioFiles.length) return;

        if (audioIndex <= audioFiles.length - 1) {
            if (!isPlaying) return;

            let audioInfo = audioFiles[audioIndex];

            handleVerseUpdate(audioInfo);

            player.current.pause();
            player.current.src = audioInfo.url;
            player.current.load();
            player.current.play();
        } else {
            player.current.pause();
            player.current.src = audioFiles[0].url;
            setActiveVerse('');
            setAudioIndex(0);
            setTimestamp(0);
            setIsPlaying(false);
    ...
    const handleGetScript = async (page: string) => {
        try {
            let payload = {
                chapter: '0',
                juz: '0',
            ...
            if (activeTab == 'chapters' || activeTab == 'verses') {
                payload.page = page;
                payload.chapter = '0';
            }

            const res = await axios.get<QurancScript>(
                QuranScriptController.url(payload),
            );
    ...
    const getReciters = async () => {
        try {
            if (reciters.length) {
                if (selectedReciter) getRecitations(selectedReciter);
                return;
            }

            const res = await axios.get<Reciters>(
                QuranRecitersController.reciters().url,
            );

            setReciters(res.data);
            setSelectedReciter(res.data.recitations[0]);
            getRecitations(res.data.recitations[0]);
        } catch (error) {
            console.error(error);
        }
    };

    const getRecitations = async (reciter: Reciter) => {
        try {
            setDisabled(true);

            const audioFileRes = await axios.get<AudioFiles>(
                QuranRecitersController.recitations({
                    reciter: reciter.id,
                }).url,
            );
    ...
    const handlePlayer = () => {
        ...
        if (audioIndex == 0) {
            player.current.src = audioFiles[0].url;
            setActiveVerse(audioFiles[0].verse_key);
        }
    ...
    const handleVerseUpdate = (audioInfo: AudioFile) => {
        setActiveVerse(audioInfo.verse_key);

        const vers = document.querySelector(
            `[data-testid="verse-arabic-${audioInfo.verse_key}"]`,
        );
    ...
    const handleReciterChange = (value: string) => {
        const reci = reciters.find((item) => item.id == +value);

        if (!reci) return;

        setSelectedReciter(reci);
    ...
    return (
        <>
                ...
                <div className="mx-5 md:mx-auto">
                    {script && chapter && (
                        <div className="flex w-full flex-col">
                            {(activeTab == 'chapters' ||
                                activeTab == 'verses') && (
                                <div className="mx-auto mt-10 flex h-fit w-full max-w-3xl flex-col gap-y-2 rounded-md bg-brand-accent p-3 shadow-md">
                                    <div className="flex justify-end" dir="rtl">
                                        <Link
                                            href={quranChapers()}
                                            className="flex gap-x-2 rounded-full bg-brand-accent-45 px-2 py-1 font-bold text-white shadow-sm hover:bg-brand-accent-50"
                                        >
                                            <List className="w-5 text-white" />
                                            <p>سورة</p>
                                        </Link>
                                    </div>
                                    <div className="items-center gap-x-5 md:flex">
                                        <h1 className="surah-name text-right text-7xl text-white md:text-left">
                                            {chapter.id
                                                .toString()
                                                .padStart(3, '00')}
                                        </h1>
                                        <div className="flex flex-col">
                                            <h2 className="text-3xl font-bold text-white">{`${chapter.id}. Surah ${(chapter as Chapter).name_simple}`}</h2>
                                            <p className="text-lg text-white">
                                                {`${(chapter as Chapter).translated_name.name}`}
                                            </p>
                                        </div>
                                    </div>
                                    <div className="flex flex-col gap-y-3 p-3 md:flex-row md:gap-x-3">
                                        <button
                                            onClick={() => handlePlayer()}
                                            className="flex w-fit cursor-pointer items-center justify-center gap-x-1 rounded-full bg-brand-accent-25 px-2 py-1 text-white hover:bg-brand-accent-45 disabled:bg-gray-50"
                                            disabled={
                                        ...
                                        {selectedReciter && (
                                            ...
                                            >
                                                <SelectTrigger className="rounded-full bg-brand-accent-25">
                            ...
                            >
                                {(activeTab == 'chapters' ||
                                    activeTab == 'verses') &&
                                    (chapter as Chapter).bismillah_pre && (
                                ...
                                <div className="max-w-lg px-2 md:max-w-3xl">
                                    {renderedPages.length > 0 &&
                                        renderedPages.map((pg) => (
                                            <div
                                                ...
                                                className={cn(
                                                    `page-${pg}`,
                                                    'quran-text page-container',
                                                )}
                                            >
                                                {script[pg]?.verses.map(
                                                    (words, verseIndex) => (
                                                        <div
                                                            key={words.id}
                                                            tabIndex={
                                                            ...
                                                            data-testid={`verse-arabic-${words.verse_key}`}
                                                            className={cn(
                                                                'inline w-full cursor-pointer transition-colors hover:text-blue-900',
                                                                activeVerse ==
                                                                    words.verse_key &&
                                                                ...
                                                                .map(
                                                                    (
                                                                        ...
                                                                        <span
                                                                            ...
                                                                            data-word-location={`${words.verse_key}:${index + pg + words.verse_number}`}
                                                                            className={cn(
                                                                                'inline-block cursor-pointer font-semibold hover:text-brand-primary',
                                                                            )}
                                                                        >
                                                                            {
                                                                                word
                                                ...
                                                {script[pg] && (
                                                    <div className="my-5 flex w-full justify-center">
                                                        <div className="w-fit rounded-md border border-gray-500 px-4 py-1">
            ...
            <div
                ref={ref}
                className="flex h-0.5 w-full justify-center bg-transparent"
            ></div>
        </>
    );
}

Quran.layout = ({ slug }: Prop) => {
    return {
        breadcrumbs: [
            {
                title: 'الرئيسية',
