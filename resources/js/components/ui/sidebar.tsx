...
import { ChevronDown, ChevronUp, PanelLeftCloseIcon, PanelLeftOpenIcon } from "lucide-react"
...
type SidebarContext = {
  ...
  isMobile: boolean
  toggleSidebar: (type?: "drawer" | "native") => void
}
...
function SidebarProvider({
  ...
  const toggleSidebar = React.useCallback((type = "native") => {
    return isMobile || type == "drawer" ? setOpenMobile((open) => !open) : setOpen((open) => !open)
    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        ...
        <SheetContent
          ...
          overlayStyle={type == "drawer" ? 'bg-transparent' : ''}
          style={
            {
...
function SidebarQuranTrigger({
  ...
  return (
    <Button
      ...
      variant="ghost"
      size="icon"
      className={cn("w-full  flex justify-start px-2 rounded-none", className)}
      onClick={(event) => {
        onClick?.(event)
    ...
    >
      {children}
      {isMobile || state === "collapsed" ? <ChevronUp /> : <ChevronDown />}
    </Button>
  )
}
